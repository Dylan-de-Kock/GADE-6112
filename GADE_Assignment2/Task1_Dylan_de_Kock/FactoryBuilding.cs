...
namespace Task1_Dylan_de_Kock
{
    class FactoryBuilding : Building
    {
        ...
        public FactoryBuilding(int x, int y, int hp, bool team, string symbol)
        ...
        public override void save()
        {
            try
            {
            FileStream outFile = new FileStream(@"Files\\FactoryBuilding.txt", FileMode.Create, FileAccess.Write);
        ...
        public Unit unitSpawn(Building b)
        {
            ...
            if (b.tm == true)
            {
                ...
                if (unitType == 1)
                {
                    ru = new RangerUnit(x, y, 3, 1, 2, 1, true, "R");
                    type = true;
                }
                else if (unitType == 2)
                {
                    mu = new MeleeUnit(x, y, 3, 2, 2, 1, true, "M");
                    type = false;
                }
            }
            ...
            else
            {
                ...
                if (unitType == 1)
                {
                    ru = new RangerUnit(x, y, 3, 1, 2, 1, false, "r");
                    type = true;
                }
                else if (unitType == 2)
                {
                    mu = new MeleeUnit(x, y, 3, 2, 2, 1, false, "m");
                    type = false;
                }
            }
